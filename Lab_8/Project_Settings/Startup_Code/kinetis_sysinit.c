...
// Reset MCU back to correct state for the lab
void setup_lab(void)
{
	// Enable the interrupt in the NVIC for FTM2
	NVICISER2 = 1;
	
	// Set clock source to internal, prescale factor to divide by 128
	// and enable interrupts on timer overflow by updating
	// Status Control (SC) register
	FTM2_SC = FTM_SC_CLKS(1) | FTM_SC_PS(7) | FTM_SC_TOIE_MASK;
	
	// Set initial value of FTM counter to 5
	FTM2_CNTIN = 5;
	// Counter overflows when it reaches MOD value 69,904
	FTM2_MOD = 0xffff;

	// Free running counter and synchronization are different
	// from TPM
	FTM2_MODE |= FTM_MODE_FTMEN_MASK;
	// Enable output compare
	FTM2_COMBINE = 0;

	// Output function run each time counter passes this value - 65,536
	FTM2_C1V = 0xf000;
	// Set output function to toggle output, i.e., switch LED on/off and
	// allow interrupts for output compare. This will cause the two
	// different interrupts to be handled by the same ISR.
	FTM2_C1SC = FTM_CnSC_MSA_MASK | FTM_CnSC_ELSA_MASK | FTM_CnSC_CHIE_MASK;
}
...
static __declspec(vectortable) tVectorTable __vect_table = { /* Interrupt vector table */
   __SP_INIT,                                              /* 0 (0x00000000) (prior: -) */
  {
   ...
   (tIsrFunc)isr_ftm2,                               /* 80 (0x00000140) (prior: -) */
   ...
   (tIsrFunc)UNASSIGNED_ISR,                               /* 120 (0x000001E0) (prior: -) */
   (tIsrFunc)UNASSIGNED_ISR                                /* 121 (0x000001E4) (prior: -) */
  }
};
